...
- `study_contract` configures the learner's Think → Plan → Reflect prompts from the lesson manifest. It preserves messy jot notes, the Feynman explanation and limit prompts, and the existing implementation handoff; it does not accept learner-entered text as proof or completion evidence.
