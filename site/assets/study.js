...
function renderStudyPanel(lessonId) {
  ...
  panel.innerHTML = `
    ...
    <p class="study-intro">Notes save locally to your study database as you work. No code is edited or run here.</p>
...
function scheduleSave(panel, lessonId, delay = 550) {
  ...
  saveTimer = window.setTimeout(async () => {
    try {
      const response = await fetch(apiPath(lessonId), {
        method: "PUT",
      ...
      setSaveState(panel, "Start local study server");
    }
  }, delay);
}
...
async function setupLessonWorkspace() {
  ...
  try {
    ...
    setSaveState(panel, "Saved locally");
  } catch {
    setSaveState(panel, "Start local study server");
  }
...
async function setupCourseHome() {
  ...
  try {
    ...
    progress.textContent = "Start the local study server to save answers and track progress.";
  }
}
